...
import type { ResponseIntelligenceLine, ResponseIntelligenceViewModel } from "./view-model";
...
function landedCostLabel(line: ResponseIntelligenceLine): string {
  if (line.price?.landedCost === undefined) return "Not finalized";
  return `${formatMoney(line.price.landedCost, line.price.currency)} ${line.price.priceBasisRaw}`;
}
...
/**
 * Response Intelligence body: vendor summary, comparability buckets, a
 * lines table that keeps "vendor quoted", "landed cost" and "normalized
 * unit price" visually distinct (never just "price"), and per-exception
 * detail cards with evidence + a clarification action. Shared by the
 * original single-vendor demo route (/analyze-response/intelligence) and
 * the RFx workspace's per-vendor Responses detail -- same underlying data
 * and logic, only the surrounding page chrome differs.
 */
export function ResponseIntelligenceView({
  intelligence,
  analystHref,
}: {
  ...
  const { vendorResponse, lines, comparability, exceptionGroups } = intelligence;

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-slate-900 dark:text-slate-50">Response Intelligence</h2>
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
            What {vendorResponse.vendor.name} quoted, what Aerchain normalized, and what still needs your judgment.
          </p>
        </div>
        <Link
          href={analystHref}
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">{vendorResponse.vendor.name}</h3>
            <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
        ...
        <ul className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {vendorResponse.documents.map((doc) => (
            <li key={doc.id} className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs dark:border-slate-800 dark:bg-slate-950">
              <p className="font-medium text-slate-800 dark:text-slate-200">{SOURCE_FORMAT_LABEL[doc.format] ?? doc.format}</p>
      ...
      {/* Comparability */}
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Comparability</h3>
        <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">How each line stands right now -- nothing here has been guessed.</p>
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          {(["safely_comparable", "needs_attention", "unresolved"] as const).map((bucket) => (
            <div key={bucket} className={`rounded-lg px-4 py-3 ring-1 ring-inset ${BUCKET_STYLE[bucket]}`}>
      ...
      {/* Lines: quoted vs. landed vs. normalized, kept visually distinct */}
      <section className="rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Lines</h3>
        </div>
          ...
          <table className="w-full min-w-[900px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:text-slate-400">
                ...
                <th className="px-5 py-3 font-medium">Vendor quoted</th>
                <th className="px-5 py-3 font-medium">Landed cost</th>
                <th className="px-5 py-3 font-medium">Normalized unit price</th>
              ...
              {lines.map((line) => (
                <tr key={line.vendorResponseLine.id} className="border-b border-slate-100 last:border-0 dark:border-slate-800">
                  <td className="px-5 py-4 align-top">
                    <p className="font-medium text-slate-900 dark:text-slate-100">{line.rfxLine ? `Line ${line.rfxLine.lineNumber}` : "Unmapped"}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{line.rfxLineProduct?.name ?? line.vendorResponseLine.rawProductDescription}</p>
                  </td>
                  <td className="px-5 py-4 align-top text-slate-700 dark:text-slate-300">
                    ...
                    {line.vendorResponseLine.rawQuantity && <p className="text-xs text-slate-500 dark:text-slate-400">Qty: {line.vendorResponseLine.rawQuantity}</p>}
                  </td>
                  <td className="px-5 py-4 align-top text-slate-700 dark:text-slate-300">{landedCostLabel(line)}</td>
                  <td className="px-5 py-4 align-top font-medium text-slate-900 dark:text-slate-100">{normalizedPriceLabel(line)}</td>
                  <td className="px-5 py-4 align-top">
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${BUCKET_STYLE[line.bucket]}`}>
                      {BUCKET_LABEL[line.bucket]}
                    </span>
                  </td>
                  <td className="px-5 py-4 align-top text-xs text-slate-600 dark:text-slate-300">{whyLabel(line)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="flex flex-col gap-4">
        <div>
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-50">Exceptions</h3>
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
            What the vendor said, what Aerchain understood, and why comparison is blocked -- with evidence and a proposed next step for each.
          </p>
        </div>
        ...
        {exceptionGroups.map(({ line, clarification }) => (
          <article key={line.vendorResponseLine.id} className="overflow-hidden rounded-lg border border-slate-200 shadow-sm dark:border-slate-800">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
              <div>
                ...
                <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{line.rfxLineProduct?.name ?? line.vendorResponseLine.rawProductDescription}</p>
              </div>
              <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${BUCKET_STYLE[line.bucket]}`}>
                {BUCKET_LABEL[line.bucket]}
              </span>
            ...
            <div className="grid gap-4 bg-white px-4 py-4 dark:bg-slate-900 sm:grid-cols-2">
              <div>
                <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">What the vendor said</h4>
                <dl className="mt-2 space-y-1.5 text-sm">
                  ...
                  {line.price && <Row term="Quoted price" value={`${formatMoney(line.price.quotedPrice, line.price.currency)} ${line.price.priceBasisRaw}`} />}
                </dl>
              </div>
              <div>
                <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">What Aerchain understood</h4>
                <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{line.unitResolution?.notes ?? "Could not be matched to a known unit."}</p>
            ...
            <div className="border-t border-slate-100 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
              <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Why comparison is blocked</h4>
                ...
                {line.exceptions.map((exception) => (
                  <li key={exception.id} className="flex flex-col gap-1 rounded-md bg-slate-50 px-3 py-2 text-xs dark:bg-slate-950 sm:flex-row sm:items-start sm:gap-3">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className={`inline-flex items-center rounded px-1.5 py-0.5 font-medium ring-1 ring-inset ${EXCEPTION_SEVERITY_STYLE[EXCEPTION_SEVERITY[exception.type]]}`}>
            ...
            <div className="border-t border-slate-100 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
              <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Missing information</h4>
              <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-slate-600 dark:text-slate-300">
                {Array.from(new Set(line.exceptions.map((exception) => MISSING_INFO_BY_EXCEPTION_TYPE[exception.type]))).map((info) => (
                  <li key={info}>{info}</li>
                ))}
              </ul>
            </div>
            ...
            <div className="border-t border-slate-100 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
              <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Recommended action</h4>
              {clarification && (
                <p className="mt-2 rounded-md bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:bg-slate-950 dark:text-slate-300">
                  <span className="font-medium text-slate-700 dark:text-slate-200">Existing clarification on file:</span> &ldquo;{clarification.question}&rdquo;
                </p>
              )}
              <div className="mt-2">
